...
public class StructureCandleHut extends StructureFeature<JigsawConfiguration> {

    public StructureCandleHut(Codec<JigsawConfiguration> codec) {
        super(codec, StructureCandleHut::createPiecesGenerator, PostPlacementProcessor.NONE);
    }
    ...
    public static void setupStructureSpawns(final StructureSpawnListGatherEvent event) {
        if(event.getStructure() == InitStructures.CANDLE_HUT.get()) {
    ...
    public static Optional<PieceGenerator<JigsawConfiguration>> createPiecesGenerator(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        ...
        if (!StructureCandleHut.isFeatureChunk(context)) {
        ...
        JigsawConfiguration newConfig = new JigsawConfiguration(() -> context.registryAccess().ownedRegistryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
                .get(new ResourceLocation(KCReference.MOD_ID, "candle_hut_pool")), 10
        );
        ...
        if(structurePiecesGenerator.isPresent()) {
            LogHelper.log(KCReference.MOD_NAME, "Candle Maker Hut at " + blockpos);
        }
